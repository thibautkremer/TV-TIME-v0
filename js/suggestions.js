...
function appendSuggestions() {
    ...
    modalSuggestionsPool.slice(0, 10).forEach(n => {
        ...
        div.innerHTML = `
            ...
            <div class="flex-1 min-w-0 cursor-pointer text-[11px]" onclick="closeModal(); openPreviewModal(${JSON.stringify(n).replace(/"/g, '&quot;')})">
                <h4 class="font-bold text-white truncate">${n.title_fr}</h4>
                <div class="text-[9px] text-gray-400"><span class="text-yellow-400 font-bold">★ ${n.rating.toFixed(1)}</span></div>
            </div>
