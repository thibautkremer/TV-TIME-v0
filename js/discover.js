...
const suggestionsObserver = new IntersectionObserver(entries => { 
    if (entries[0].isIntersecting) { /* Logique de pagination si besoin */ } 
});
...
async function renderSuggestions(currentId) {
    ...
    try {
        const baseMedia = library.find(i => i.id === currentId);
        if (!baseMedia) return;

        // 1. Appel à l'API 'recommendations' de TMDB (le plus pertinent)
        ...
        // 2. Si aucune recommandation (ex: média très récent), on cherche des médias similaires
        ...
        // 3. Transformation et filtrage des doublons déjà en bibliothèque
...
function appendSuggestions() {
    ...
    // On prend les 10 premières recommandations
