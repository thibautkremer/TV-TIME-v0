...
function createMediaCard(media, context = 'library') {
    ...
    const isAnime = (m.genres || []).includes(16) || (m.genres || []).includes('Anime') || (m.genres || []).includes('Animation') || m.original_language === 'ja';
...
function refreshGrids() {
    const grids = [
        { id: 'searchResults', arr: searchResults, type: window.currentSearchType || 'series', ctx: 'search' },
    ...
    grids.forEach(g => {
        ...
        if (el && !el.parentElement.classList.contains('hidden')) {
            ...
            const filtered = g.arr.filter(r => {
                const isAnime = (r.genres || []).includes(16) || (r.genres || []).includes('Anime') || (r.genres || []).includes('Animation') || r.original_language === 'ja';
                if (g.type === 'anime') return isAnime;
                if (g.type === 'series') return r.type === 'series' && !isAnime;
                return r.type === g.type;
            });
            
            const frag = document.createDocumentFragment();
            filtered.forEach(m => {
                if (typeof createMediaCard === 'function') frag.appendChild(createMediaCard(m, g.ctx));
            });
            el.appendChild(frag);
        }
    });
